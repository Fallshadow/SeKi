...
{
    public static class MiddleWindowHelper
    {
        ...
        //prevent from outside the screen
        public static void AutoFixPos(this EditorWindow window, Rect rect)
        {
            //var pos = window.position;

            var pos = rect;

            var maxHeight = pos.y + pos.height;
            var maxWidth = pos.x + pos.width;

            if (maxHeight > Screen.height)
            {
                pos.y -= (maxHeight - Screen.height + 25f);
            }
            //有双屏的可能性 横的就不考虑了
//            if (maxWidth > Screen.width)
//            {
//                pos.x -= (maxWidth - Screen.width + 25f);
//            }
